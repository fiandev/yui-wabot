...
export const convert: Command = {
  ...
  cmd: ["convert"],
  category: "Currency",
  description: "allows you to convert between various fiat currencies and cryptocurrencies. Users can specify an amount, the source currency, and the target currency to get the converted value. It fetches real-time exchange rates and cryptocurrency prices from multiple reliable sources to ensure accuracy. This is useful for financial applications, personal finance tools, and any service requiring up-to-date currency conversions..",
  usage: "Usage: .convert <amount> | from=<value> | to=<value>\nExample: .convert 100 | from=USD | to=IDR",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["amount","from","to"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/currency/convert?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'convert':", error);
