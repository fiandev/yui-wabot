...
export const whisper: Command = {
  ...
  usage: "Usage: .whisper <audioUrl> | model=<value>\nExample: .whisper https://github.com/Azure-Samples/cognitive-services-speech-sdk/raw/master/samples/cpp/windows/console/samples/enrollment_audio_katie.wav | model=@cf/openai/whisper",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["audioUrl","model"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/cf/whisper?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'whisper':", error);
