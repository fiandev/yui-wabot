...
export const imageClassification: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["imageUrl","model"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/cf/image-classification?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'imageClassification':", error);
