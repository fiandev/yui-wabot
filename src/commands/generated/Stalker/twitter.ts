...
export const twitter: Command = {
  ...
  cmd: ["twitter"],
  category: "Stalker",
  description: "allows you to retrieve public profile information for a specified Twitter (X) user using their username as a query parameter. It fetches detailed user data including their ID, username, display name, verification status, biography, location, account creation date, and various statistics such as tweet count, followers, following, likes, and media count. It also provides URLs for their profile image and banner. This is useful for applications requiring Twitter user data for display, analysis, or integration..",
  usage: "Usage: .twitter <user>\nExample: .twitter siputzx",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/stalk/twitter?${searchParams.toString()}`;
