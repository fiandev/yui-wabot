...
export const xnxx: Command = {
  ...
  usage: "Usage: .xnxx <title> | image=<value>\nExample: .xnxx Lari ada wibu | image=https://files.catbox.moe/zhsks3.jpg",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["title","image"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/canvas/xnxx?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'xnxx':", error);
