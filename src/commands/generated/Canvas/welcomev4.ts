...
export const welcomev4: Command = {
  name: "welcomev4",
  cmd: ["welcomev4"],
  category: "Canvas",
  ...
  usage: "Usage: .welcomev4 <avatar> | background=<value> | description=<value>\nExample: .welcomev4 https://i.ibb.co/1s8T3sY/48f7ce63c7aa.jpg | background=https://i.ibb.co/4YBNyvP/images-76.jpg | description=Welcome friend!",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["avatar","background","description"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/canvas/welcomev4?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'welcomev4':", error);
