...
export const goodbyev3: Command = {
  name: "goodbyev3",
  cmd: ["goodbyev3"],
  category: "Canvas",
  ...
  usage: "Usage: .goodbyev3 <username> | avatar=<value>\nExample: .goodbyev3 John | avatar=https://i.ibb.co/1s8T3sY/48f7ce63c7aa.jpg",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["username","avatar"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/canvas/goodbyev3?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'goodbyev3':", error);
