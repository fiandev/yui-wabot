...
export const welcomev2: Command = {
  name: "welcomev2",
  cmd: ["welcomev2"],
  category: "Canvas",
  ...
  usage: "Usage: .welcomev2 <username> | guildName=<value> | memberCount=<value> | avatar=<value> | background=<value>\nExample: .welcomev2 John | guildName=Siputzx Api | memberCount=150 | avatar=https://i.ibb.co/1s8T3sY/48f7ce63c7aa.jpg | background=https://i.ibb.co/4YBNyvP/images-76.jpg",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["username","guildName","memberCount","avatar","background"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/canvas/welcomev2?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'welcomev2':", error);
