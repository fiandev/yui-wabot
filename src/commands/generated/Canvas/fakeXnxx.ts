...
export const fakeXnxx: Command = {
  name: "fakeXnxx",
  cmd: ["fakeXnxx"],
  category: "Canvas",
  description: "Generate a fake XNXX comment image with a specified name, quote, and optional like/dislike counts. allows users to create humorous or satirical images resembling XNXX comments, ideal for social media content or creative projects. Input parameters include the commenter's name, the comment text (quote), and the number of likes and dislikes. The output is a PNG image buffer..",
  usage: "Usage: .fakeXnxx <name> | quote=<value> | likes=<value> | dislikes=<value>\nExample: .fakeXnxx Nelson Mandela | quote=Keberanian bukanlah tidak adanya ketakutan, tetapi kemenangan atas ketakutan itu. | likes=2 | dislikes=0",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["name","quote","likes","dislikes"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/canvas/fake-xnxx?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'fakeXnxx':", error);
