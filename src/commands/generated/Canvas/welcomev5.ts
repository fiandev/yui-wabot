...
export const welcomev5: Command = {
  name: "welcomev5",
  cmd: ["welcomev5"],
  category: "Canvas",
  ...
  usage: "Usage: .welcomev5 <username> | guildName=<value> | memberCount=<value> | avatar=<value> | background=<value> | quality=<value>\nExample: .welcomev5 Zero Two | guildName=Siputzx Community | memberCount=219 | avatar=https://i.ibb.co/1s8T3sY/48f7ce63c7aa.jpg | background=https://i.ibb.co/4YBNyvP/mountain-sunset.jpg | quality=90",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["username","guildName","memberCount","avatar","background","quality"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/canvas/welcomev5?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'welcomev5':", error);
