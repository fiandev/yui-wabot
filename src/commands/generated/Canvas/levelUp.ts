...
export const levelUp: Command = {
  ...
  usage: "Usage: .levelUp <backgroundURL> | avatarURL=<value> | fromLevel=<value> | toLevel=<value> | name=<value>\nExample: .levelUp https://i.ibb.co.com/2jMjYXK/IMG-20250103-WA0469.jpg | avatarURL=https://avatars.githubusercontent.com/u/159487561?v=4 | fromLevel=0 | toLevel=1 | name=putu",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["backgroundURL","avatarURL","fromLevel","toLevel","name"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/canvas/level-up?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'levelUp':", error);
