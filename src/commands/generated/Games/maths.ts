...
export const maths: Command = {
  name: "maths",
  cmd: ["maths"],
  category: "Games",
  description: "generates a random math problem based on a specified difficulty level. Users can choose from various levels like 'noob', 'easy', 'medium', 'hard', 'extreme', and 'impossible' to get a problem tailored to their skill. Each problem includes the mathematical expression, the chosen difficulty mode, a time limit for solving, a bonus for correct answers, and the correct result. This endpoint is ideal for educational apps, brain training games, or any platform requiring dynamic arithmetic challenges..",
  usage: "Usage: .maths <level>\nExample: .maths input utama",
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/games/maths?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'maths':", error);
