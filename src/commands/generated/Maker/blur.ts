...
export const blur: Command = {
  ...
  cmd: ["blur"],
  category: "Maker",
  description: "allows you to apply a blur effect to an image provided via a URL. The image will be fetched through a proxy to handle potential cross-origin issues. The processed image will be returned as a PNG buffer. This can be used for various creative applications, such as creating background effects, artistic filters, or to obscure sensitive information in images..",
  usage: "Usage: .blur <image>\nExample: .blur https://i.ibb.co/9rtTrVy/download-1.jpg",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/m/blur?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'blur':", error);
