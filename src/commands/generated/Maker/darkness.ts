...
export const darkness: Command = {
  ...
  usage: "Usage: .darkness <image> | amount=<value>\nExample: .darkness https://i.ibb.co/9rtTrVy/download-1.jpg | amount=50",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["image","amount"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/m/darkness?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'darkness':", error);
