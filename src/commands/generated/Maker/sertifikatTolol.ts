...
export const sertifikatTolol: Command = {
  name: "sertifikatTolol",
  cmd: ["sertifikatTolol"],
  category: "Maker",
  description: "generates a 'Sertifikat Tolol' (Fool's Certificate) image with custom text. Users can provide a string of text via a query parameter, which will be rendered onto a pre-designed certificate template. This feature is primarily for entertainment purposes, allowing users to create humorous or ironic 'certificates' for various scenarios. The generated image is returned in JPEG format..",
  usage: "Usage: .sertifikatTolol <text>\nExample: .sertifikatTolol lorem ipsum!",
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/m/sertifikat-tolol?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'sertifikatTolol':", error);
