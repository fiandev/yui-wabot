...
export const carbonify: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["input","title"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/m/carbonify?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'carbonify':", error);
