...
export const textpro: Command = {
  name: "textpro",
  cmd: ["textpro"],
  category: "Maker",
  description: "allows you to generate custom images using textpro.me effects by providing a TextPro URL and up to two text inputs. It supports various text effects available on the textpro.me website. The endpoint handles the entire process, including reCAPTCHA bypass, form submission, and image generation, returning the final image as a binary buffer. This is useful for creating personalized images programmatically, such as logos, banners, or stylized text for social media and web applications..",
  usage: "Usage: .textpro <url> | text1=<value> | text2=<value>\nExample: .textpro https://textpro.me/create-neon-devil-wings-text-effect-online-free-1014.html | text1=Hello | text2=World",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["url","text1","text2"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/m/textpro?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'textpro':", error);
