...
export const tweet: Command = {
  ...
  cmd: ["tweet"],
  category: "Maker",
  description: "allows you to generate a realistic-looking tweet image by providing various parameters such as profile picture, display name, username, tweet content, an optional image within the tweet, theme (dark, light, or dim), retweet count, quote count, like count, and the client from which the tweet was posted. It is useful for creating mockups, social media content, or testing purposes..",
  usage: "Usage: .tweet <profile> | name=<value> | username=<value> | tweet=<value> | image=<value> | theme=<value> | retweets=<value> | quotes=<value> | likes=<value> | client=<value>\nExample: .tweet https://avatars.githubusercontent.com/u/159487561?v=4 | name=siputzx | username=siputzx | tweet=Hello World | image=null | theme=contoh | retweets=1000 | quotes=200 | likes=5000 | client=Twitter for iPhone",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["profile","name","username","tweet","image","theme","retweets","quotes","likes","client"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/m/tweet?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'tweet':", error);
