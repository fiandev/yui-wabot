...
export const skiplink: Command = {
  name: "skiplink",
  cmd: ["skiplink"],
  category: "Tools",
  description: "bypasses link shorteners using bypass.city service with Turnstile token solving. It takes a shortener URL and returns the bypassed direct link..",
  usage: "Usage: .skiplink <url>\nExample: .skiplink https://linkvertise.com/514008/hydrogen-gateway-1",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/tools/skiplink?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'skiplink':", error);
