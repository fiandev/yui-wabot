...
export const translate: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["text","source","target"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/tools/translate?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'translate':", error);
