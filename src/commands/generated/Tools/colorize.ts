...
export const colorize: Command = {
  ...
  cmd: ["colorize"],
  category: "Tools",
  description: "colorizes a grayscale image using a provided URL. Users can submit the URL of a grayscale image, and the API will process it to add color, returning the colorized image as a WebP buffer. This tool is ideal for restoring old photos or enhancing black and white images with realistic colors..",
  usage: "Usage: .colorize <url>\nExample: .colorize https://files.catbox.moe/258vhm.jpg",
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/tools/colorize?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'colorize':", error);
