...
export const tts: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["text","voice","rate","pitch","volume"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/tools/tts?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'tts':", error);
