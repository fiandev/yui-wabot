...
export const upscale: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["image","scale"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/iloveimg/upscale?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'upscale':", error);
