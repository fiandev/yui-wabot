...
export const stabilityai: Command = {
  name: "stabilityai",
  cmd: ["stabilityai"],
  category: "AI Image",
  description: "allows users to generate images using the StabilityAI model. By providing a descriptive prompt, users can create diverse images, from realistic photos to artistic illustrations. The API processes the prompt and returns the generated image in JPEG format. This functionality is ideal for creative applications, content generation, and prototyping, enabling quick visual asset creation..",
  usage: "Usage: .stabilityai <prompt>\nExample: .stabilityai a cute cat sitting on a couch",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/ai/stabilityai?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'stabilityai':", error);
