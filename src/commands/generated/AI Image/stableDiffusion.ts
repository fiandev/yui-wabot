...
export const stableDiffusion: Command = {
  name: "stableDiffusion",
  cmd: ["stableDiffusion"],
  category: "AI Image",
  description: "leverages the Stable Diffusion model to generate images based on a textual prompt provided via query parameters. Users can specify a detailed description of the desired image, and the AI will create a corresponding visual output. This functionality is highly versatile, suitable for generating various types of images for creative projects, content creation, or visual design. The endpoint returns the generated image as a JPEG file..",
  usage: "Usage: .stableDiffusion <prompt>\nExample: .stableDiffusion a futuristic city at sunset",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/ai/stable-diffusion?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'stableDiffusion':", error);
