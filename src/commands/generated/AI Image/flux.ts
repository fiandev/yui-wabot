...
export const flux: Command = {
  name: "flux",
  cmd: ["flux"],
  category: "AI Image",
  description: "allows users to generate images based on a textual prompt using the Flux model. By providing a descriptive prompt, users can create various images, ranging from abstract concepts to specific scenes. The API processes the prompt and returns the generated image in PNG format. It is ideal for creative applications, content generation, and visual prototyping. The generated images can be used in web applications, design projects, or for personal creative endeavors..",
  usage: "Usage: .flux <prompt>\nExample: .flux a cyberpunk lizard",
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/ai/flux?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'flux':", error);
