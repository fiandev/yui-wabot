...
export const kecocokanNamaPasangan: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["nama1","nama2"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/primbon/kecocokan_nama_pasangan?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'kecocokanNamaPasangan':", error);
