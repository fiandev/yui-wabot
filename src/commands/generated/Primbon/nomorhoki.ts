...
export const nomorhoki: Command = {
  ...
  async execute(sock, msg) {
    ...
    apiParams['phoneNumber'] = mainQuery;
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["phoneNumber"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/primbon/nomorhoki?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'nomorhoki':", error);
