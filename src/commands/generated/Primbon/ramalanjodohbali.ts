...
export const ramalanjodohbali: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["nama1","tgl1","bln1","thn1","nama2","tgl2","bln2","thn2"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/primbon/ramalanjodohbali?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'ramalanjodohbali':", error);
