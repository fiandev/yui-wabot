...
export const soundcloud: Command = {
  name: "soundcloud",
  cmd: ["soundcloud"],
  category: "Downloader",
  description: "allows you to download audio from a SoundCloud track by providing its URL as a query parameter. It uses the SoundCloud API to resolve the track information, including its title, artist, duration, thumbnail, and the direct progressive download link. The endpoint handles various SoundCloud URL formats, including mobile links, and provides robust error handling for invalid URLs, tracks not found, or issues during the download link retrieval process. The output is a structured JSON object containing all relevant track details and the direct audio download URL..",
  usage: "Usage: .soundcloud <url>\nExample: .soundcloud https://m.soundcloud.com/teguh-hariyadi-652597010/anji-dia",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/d/soundcloud?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'soundcloud':", error);
