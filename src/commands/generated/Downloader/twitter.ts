...
export const twitter: Command = {
  ...
  cmd: ["twitter"],
  category: "Downloader",
  description: "allows users to download videos from Twitter by providing the video's URL. It fetches the necessary token from the SnapTwitter website, constructs a POST request with the video URL and token, and then scrapes the resulting HTML to extract the image URL, download link, video title, and description. This tool is useful for archiving Twitter videos or sharing them across platforms where direct Twitter links might not be easily accessible or playable..",
  usage: "Usage: .twitter <url>\nExample: .twitter https://twitter.com/9GAG/status/1661175429859012608",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/d/twitter?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'twitter':", error);
