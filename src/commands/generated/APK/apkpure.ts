...
export const apkpure: Command = {
  name: "apkpure",
  cmd: ["apkpure"],
  category: "APK",
  description: "allows you to search for Android applications on apkpure.com. It uses Puppeteer with Stealth Plugin to simulate a real browser and bypass detection, scraping application details such as title, developer, download link, icon, and rating information from the search results page. The API supports both standard search results and branded application listings on APKPure. This is useful for developers who need to programmatically access and retrieve information about applications available on APKPure..",
  usage: "Usage: .apkpure <search>\nExample: .apkpure free fire",
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/apk/apkpure?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'apkpure':", error);
