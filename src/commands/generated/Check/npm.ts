...
export const npm: Command = {
  ...
  async execute(sock, msg) {
    ...
    apiParams['packageName'] = mainQuery;
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["packageName"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/check/npm?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'npm':", error);
