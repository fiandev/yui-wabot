...
export const resi: Command = {
  name: "resi",
  cmd: ["resi"],
  category: "Check",
  description: "allows you to track your package shipment using tracking number (resi) and courier name via query parameters. It retrieves shipping status, history, and other tracking details from various Indonesian shipping couriers..",
  usage: "Usage: .resi <resi> | courier=<value>\nExample: .resi 1234567890 | courier=JNE",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["resi","courier"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/check/resi?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'resi':", error);
