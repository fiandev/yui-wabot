...
export const pinterest: Command = {
  name: "pinterest",
  cmd: ["pinterest"],
  category: "Search",
  description: "allows you to search for various media types on Pinterest, including images, videos, and GIFs. You can specify a search query to find relevant content. The results will include details such as the Pinterest pin URL, original link, creation date, image URL, video URL (if applicable), GIF URL (if applicable), title, description, and information about the pinner and board. This is useful for applications requiring visual content search from Pinterest..",
  usage: "Usage: .pinterest <query> | type=<value>\nExample: .pinterest cat | type=contoh",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["query","type"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/s/pinterest?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'pinterest':", error);
