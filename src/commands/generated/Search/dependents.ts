...
export const dependents: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["url","begin","end"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/github/dependents?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'dependents':", error);
