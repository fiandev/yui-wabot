...
export const _8font: Command = {
  name: "_8font",
  cmd: ["_8font"],
  category: "Search",
  description: "allows you to search for fonts available on 8font.com. You can specify a search query to find fonts matching your criteria and also paginate through the results. This is useful for developers who need to integrate font search capabilities into their applications or for designers looking for specific font styles..",
  usage: "Usage: ._8font <query> | page=<value>\nExample: ._8font cartoon | page=1",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["query","page"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/s/8font?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command '_8font':", error);
