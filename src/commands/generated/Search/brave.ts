...
export const brave: Command = {
  name: "brave",
  cmd: ["brave"],
  category: "Search",
  description: "performs a web search using the Brave search engine. It takes a search query as input and returns a list of search results, including the title, description, URL, image URL (if available), site name, and publication date for each result. This is useful for applications that need to integrate a privacy-focused web search capability..",
  usage: "Usage: .brave <query>\nExample: .brave apa itu nodejs",
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/s/brave?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'brave':", error);
