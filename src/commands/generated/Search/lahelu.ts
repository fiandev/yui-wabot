...
export const lahelu: Command = {
  name: "lahelu",
  cmd: ["lahelu"],
  category: "Search",
  description: "allows users to search for posts on Lahelu.com, a social media platform, by providing a search query. It retrieves and formats information about relevant posts, including their unique ID, associated media (image or video), media thumbnail, the username and avatar of the user who posted it, and the creation timestamp. This endpoint is useful for integrating Lahelu's content into other applications, for content discovery, or for analyzing popular trends on the platform..",
  usage: "Usage: .lahelu <query>\nExample: .lahelu drak",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/s/lahelu?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'lahelu':", error);
