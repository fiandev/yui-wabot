...
export const gitagram: Command = {
  name: "gitagram",
  cmd: ["gitagram"],
  category: "Search",
  description: "allows users to search for music chords on Gitagram by providing a music title or artist. It scrapes the search results from Gitagram and then retrieves the detailed chord information for each result, providing a comprehensive set of data including the song title, artist, direct link to the chords, type of content, and the full chord sheet. This API is useful for musicians, developers building music applications, or anyone looking to quickly access guitar or piano chords for a specific song..",
  usage: "Usage: .gitagram <search>\nExample: .gitagram sekuat hatimu",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/s/gitagram?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'gitagram':", error);
