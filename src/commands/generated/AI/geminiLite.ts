...
export const geminiLite: Command = {
  name: "geminiLite",
  cmd: ["geminiLite"],
  category: "AI",
  ...
  usage: "Usage: .geminiLite <prompt> | model=<value> | imgUrl=<value>\nExample: .geminiLite What is the capital of France? | model=gemini-2.0-flash-lite | imgUrl=contoh",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["prompt","model","imgUrl"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/ai/gemini-lite?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'geminiLite':", error);
