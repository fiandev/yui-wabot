...
export const gptOss: Command = {
  ...
  usage: "Usage: .gptOss <prompt> | content=<value>\nExample: .gptOss You are a helpful assistant. | content=Tell me a joke.",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["prompt","content"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/ai/gpt-oss?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'gptOss':", error);
