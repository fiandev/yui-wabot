...
export const bibleai: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["question","translation"].includes(key.trim())) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/ai/bibleai?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'bibleai':", error);
