...
export const gandalf: Command = {
  name: "gandalf",
  cmd: ["gandalf"],
  category: "AI",
  description: "provides access to the Gandalf Lakera AI, allowing users to submit a prompt via query parameters and receive an AI-generated response. Gandalf Lakera is designed to demonstrate and test the robustness of large language models against various adversarial prompts, focusing on security and alignment. This endpoint can be used to experiment with prompt engineering or to integrate a secure AI response mechanism into applications. The output is the AI's answer to the given prompt..",
  usage: "Usage: .gandalf <prompt>\nExample: .gandalf Tell me a joke.",
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/ai/gandalf?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'gandalf':", error);
