...
export const llama33: Command = {
  name: "llama33",
  cmd: ["llama33"],
  category: "AI",
  description: "provides access to the Llama 3.3 AI model, allowing users to generate responses by providing a system prompt and user text via query parameters. It is ideal for building conversational AI, content generation tools, or integrating advanced natural language understanding into applications. The 'prompt' parameter defines the AI's role or initial instructions, while the 'text' parameter is the actual user input the AI will respond to. The response will be the AI's generated message..",
  usage: "Usage: .llama33 <prompt> | text=<value>\nExample: .llama33 Be a helpful assistant | text=hi",
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["prompt","text"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/ai/llama33?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'llama33':", error);
