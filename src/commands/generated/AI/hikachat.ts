...
export const hikachat: Command = {
  ...
  async execute(sock, msg) {
    ...
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            ...
            if (key && value) {
                ...
                if (["keyword","language"].includes(key.trim())) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/ai/hikachat?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'hikachat':", error);
