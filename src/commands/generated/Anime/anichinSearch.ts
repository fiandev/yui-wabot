...
export const anichinSearch: Command = {
  name: "anichinSearch",
  cmd: ["anichinSearch"],
  category: "Anime",
  description: "allows users to search for anime on the Anichin website. By providing a search query, users can retrieve a list of anime titles along with their type, current status, a direct link to the anime page, and an image thumbnail. This is useful for quickly finding information about specific anime or exploring available titles on Anichin. The endpoint handles various anime categories and provides relevant details for each search result..",
  usage: "Usage: .anichinSearch <query>\nExample: .anichinSearch naga",
  ...
  async execute(sock, msg) {
    ...
    try {      
      ...
      const url = `${BASE_URL}/api/anime/anichin-search?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'anichinSearch':", error);
