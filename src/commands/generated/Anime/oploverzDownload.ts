...
export const oploverzDownload: Command = {
  name: "oploverzDownload",
  cmd: ["oploverzDownload"],
  category: "Anime",
  description: "provides download links and details for anime episodes from Oploverz. Users can retrieve information such as the episode title, release date, streaming iframe source, and a structured list of download links categorized by server and quality. This is useful for developers building anime streaming or download applications that require direct access to episode resources from Oploverz. The API scrapes the Oploverz website to extract the relevant data, ensuring up-to-date links..",
  usage: "Usage: .oploverzDownload <url>\nExample: .oploverzDownload https://oploverz.org/anime/captain-tsubasa-season-2-junior-youth-hen-1-episode-30-subtitle-indonesia/",
  ...
  async execute(sock, msg) {
    ...
    try {
      ...
      const url = `${BASE_URL}/api/anime/oploverz-download?${searchParams.toString()}`;
      ...
      console.error("Error saat menjalankan command 'oploverzDownload':", error);
