...
import { enableEphemeralTime } from "@/utils/ephemeral-time";
...
(async () => {
  ...
  const startBot = async () => {
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
      ...
      await enableEphemeralTime(sock, sender.jid);
      ...
      const cmd = matchs[0];
      const args = matchs[1] || "";
      const splitedArgs = args
        .split(bot.argsSeparator)
        .map((v: string) => v.trim())
